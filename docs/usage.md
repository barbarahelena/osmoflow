...
```bash
nextflow run barbarahelena/osmoflow --input ./samplesheet.csv --outdir ./results  -profile docker
```
...
```bash
nextflow run barbarahelena/osmoflow -profile docker -params-file params.yaml
```
...
```bash
nextflow pull barbarahelena/osmoflow
```
...
First, go to the [barbarahelena/osmoflow releases page](https://github.com/barbarahelena/osmoflow/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
